...
const weatherController = {
    ...
    getWeatherByGardenID: (req,res) => {
        const {garden_id} = req.params;
        WeatherData.getByGID(garden_id, (err, weather) => {
            if(err) return res.status(500).json({message: "Error finding weather" + err.message });
            if(!weather) return res.status(404).json({message: "No weather in this Garden found"});
            res.json(weather);
        });
    },
