...
const guideController = {
  ...
  getGuideByPartnerId: (req, res) => {
    const { partner_id } = req.params;
    Guide.getByPartnerId(partner_id, (err, guide) => {
      if (err) return res.status(500).json({ message: "Error finding guide by partner id: " + err.message });
      if (!guide) return res.status(404).json({ message: "Guide with this partner is not found" });
      res.json(guide);
    });
  },
